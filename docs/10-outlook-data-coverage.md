...
미국 금리는 기존 엑셀 값이 FRED `FEDFUNDS` 월평균 EFFR임을 129개월 전체에서 확인했고,
일별 EFFR와 FOMC 목표범위를 분리한 NGIP job 및 OUTLOOK 조회 어댑터까지 구현·검증했다.
...
| 아시아 LNG 수입 | JODI 연결 구현·배포 대기 | 한국·중국·일본·대만 국가별 `IMPLNG`. 지역 집계와 국가별 결측·품질등급 처리는 분석 단계에서 수행 |
| 유럽 LNG 수입 | JODI 연결 구현·배포 대기 | 유럽 국가별 `IMPLNG`. EU/유럽 포함국가 집계 정의는 별도 필요 |
| 글로벌 LNG 수출 | JODI 연결 구현·배포 대기 | 주요 수출국 `EXPLNG` 합산 가능. 미제출국·누락월 때문에 공식 세계 총계로 간주하지 않음 |
...
정리했다. 국가별 월간 가스 수급은 JODI-Gas로 확정했다. JODI가 제공하지 않는 양자
교역·설비용량과 기온·선박 인도량의 구체적 소스/시리즈 선정은 후속 확인 대상이다.
...
6. 가격계열의 연결·월 집계·시차 정의는 후속 단계에서 확정한다.

유럽 재기화와 글로벌 액화 프로젝트 원천·snapshot 설계의 상세 내용은
