...
국가별 저장량은 `CLOSTLV`를 사용한다. `STOCKCH`는 월말과 월초의 차이이므로 재고 수준과
별도 변수다. 2026-09-22 공식 전체 파일에서 M3 `CLOSTLV`는 65개국을 제공한다. JP·KR·US와
유럽 주요국은 장기 품질등급 1 이력이 있지만 CN·AU·QA는 없으므로 누락을 0으로 채우지 않는다.
JODI 정의상 이는 국가 영토 내 recoverable gas 월말 총량이며 LNG 탱크와 지하저장을
구분하지 않는다.
